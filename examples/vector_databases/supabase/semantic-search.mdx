...
This guide uses JavaScript to generate embeddings, but you can easily modify it to use any [language supported by OpenAI](https://platform.khulnasoft.com/docs/libraries).
...
Next we'll generate an OpenAI embedding using [`text-embedding-ada-002`](https://platform.khulnasoft.com/docs/guides/embeddings/embedding-models):
...
Remember that you will need an [OpenAI API key](https://platform.khulnasoft.com/api-keys) to interact with the OpenAI API. You can pass this as an environment variable called `STARTGPT_API_KEY`, or manually set it when you instantiate your OpenAI client:
...
For more information on OpenAI embeddings, read the [Embedding](https://platform.khulnasoft.com/docs/guides/embeddings) docs.
