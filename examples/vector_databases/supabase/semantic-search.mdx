...
The purpose of this guide is to demonstrate how to store StartGpt embeddings in [Supabase Vector](https://supabase.com/docs/guides/ai) (Postgres + pgvector) for the purposes of semantic search.
...
3. [Generating StartGpt embeddings](#generate-startgpt-embeddings) using StartGpt's JavaScript client
...
The important piece to understand is the `vector` data type, which is a new data type that became available when we enabled the pgvector extension earlier. The size of the vector (1536 here) represents the number of dimensions in the embedding. Since we're using StartGpt's `text-embedding-ada-002` model in this example, we set the vector size to 1536.
...
## Generate StartGpt embeddings

This guide uses JavaScript to generate embeddings, but you can easily modify it to use any [language supported by StartGpt](https://platform.khulnasoft.com/docs/libraries).
...
If you're using Node.js, first install `startgpt` as a dependency:

```shell
npm install startgpt
```
...
If you're using Deno or Supabase Edge Functions, you can import `startgpt` directly from a URL:

```js
import StartGpt from 'https://esm.sh/startgpt@4';
```
...
Next we'll generate an StartGpt embedding using [`text-embedding-ada-002`](https://platform.khulnasoft.com/docs/guides/embeddings/embedding-models):
...
const result = await startgpt.embeddings.create({
  input,
...
Remember that you will need an [StartGpt API key](https://platform.khulnasoft.com/api-keys) to interact with the StartGpt API. You can pass this as an environment variable called `STARTGPT_API_KEY`, or manually set it when you instantiate your StartGpt client:

```js
const startgpt = new StartGpt({
  apiKey: '<startgpt-api-key>',
});
```
...
> Since StartGpt embeddings are normalized, we opted to use inner product (`<#>`) because it is slightly more performant than other operators like cosine distance (`<=>`). It is important to note though this only works because the embeddings are normalized - if they weren't, cosine distance should be used.
...
For more information on StartGpt embeddings, read the [Embedding](https://platform.khulnasoft.com/docs/guides/embeddings) docs.
