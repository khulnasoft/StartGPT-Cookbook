...
StartGpt functions enable your app to take action based on user inputs. This means that it can, e.g., search the web, send emails, or book tickets on behalf of your users, making it more powerful than a regular chatbot.

In this tutorial, you will build an app that uses StartGpt functions along with the latest version of the Node.js SDK. The app runs in the browser, so you only need a code editor and, e.g., VS Code Live Server to follow along locally. Alternatively, write your code directly in the browser via [this code playground at Scrimba.](https://scrimba.com/scrim/c6r3LkU9)
...
StartGpt doesn't execute any code for you. It just tells your app which
...
## Importing the SDK and authenticating with StartGpt

We start by importing the StartGpt SDK at the top of our JavaScript file and authenticate with our API key, which we have stored as an environment variable.

```js
import StartGpt from "startgpt";

const startgpt = new StartGpt({
  apiKey: process.env.STARTGPT_API_KEY,
...
## Describing our functions for StartGpt

For StartGpt to understand the purpose of these functions, we need to
...
We also need to define a `messages` array. This will keep track of all of the messages back and forth between our app and StartGpt.

The first object in the array should always have the `role` property set to `"system"`, which tells StartGpt that this is how we want it to behave.
...
We start by pushing the `userInput` to the messages array. This time, we set the `role` to `"user"`, so that StartGpt knows that this is the input from the user.
...
async function agent(userInput) {
  ...
  const response = await startgpt.chat.completions.create({
    model: "gpt-4",
    messages: messages,
    tools: tools,
  });
...
When we run the code above, we see the response from StartGpt logged out
...
```js
{
    ...
    choices: [{
        ...
        logprobs: null,
        finish_reason: "tool_calls" // StartGpt wants us to call a function
    }],
    usage: {
        prompt_tokens: 134,
        completion_tokens: 6,
...
## Turning the StartGpt response into a function call
...
via bracket notation and the string we got back from StartGpt, like this:
...
We're also grabbing ahold of any arguments StartGpt wants us to pass into
...
Notice that the `role` is set to `"function"`. This tells StartGpt
that the `content` parameter contains the result of the function call
and not the input from the user.

At this point, we need to send a new request to StartGpt with this updated
...
You've now built an AI agent using StartGpt functions and the Node.js SDK! If you're looking for an extra challenge, consider enhancing this app. For example, you could add a function that fetches up-to-date information on events and activities in the user's location.
